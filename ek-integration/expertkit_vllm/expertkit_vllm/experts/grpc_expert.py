...
class GrpcExpert(PPMissingLayer):
    ...
    if os.environ.get("EK_WITH_VLLM_MINDSPORE") == "1":
        def forward():
            raise NotImplementedError(
                "GrpcExpert forward method is not implemented for Mindspore"
            )
    else:
    ...
    if os.environ.get("EK_WITH_VLLM_MINDSPORE") == "1":
        def construct(
            ...
            # Apply softmax first, then take topk
            router_probs = F.softmax(router_logits, dim=-1)
            routing_weights, routing_indices = mint.topk(
            ...
            if batch_size > 32:  # Only consider optimization for larger batches
                # Use hash or reduced precision to detect duplicates
                hidden_hash = mint.round(hidden_states * 1000).int()  # Reduce precision
                unique_hash, inverse_indices = mint.unique(
                    hidden_hash, dim=0, return_inverse=True
                )
                unique_batch_size = unique_hash.shape[0]
                should_optimize = unique_batch_size < 0.7 * batch_size  # Adjust threshold
            ...
            if not should_optimize:
                ...
                unique_hidden = hidden_states[mint.unique(inverse_indices)]
            ...
            # Compute weighted sum: [batch_size, hidden_dim]
            output = mint.sum(expanded_weights * expert_outputs, dim=1)
            return output
    else:
        def construct():
            raise NotImplementedError(
                "GrpcExpert construct method is not implemented for PyTorch"
            )
