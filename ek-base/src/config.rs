...
///
/// These control how quickly the system detects a dead worker and refreshes
/// placement state after node removal.
#[derive(Debug, Deserialize, Clone)]
#[allow(unused)]
pub struct FaultDetectionSettings {
    ...
    /// Interval between database placement-state polling passes. (default: 5)
...
"#
    ...
    fn basic_test() {
        ...
        assert_eq!(res.controller.ports.intra, 5001);
    }

    #[test]
    fn test_env_override() {
        let example_yaml = get_example_config();
        unsafe { std::env::set_var("EK_CONTROLLER_LISTEN", "127.0.0.2") };
        let config = config::Config::builder()
            .add_source(File::from_str(example_yaml, FileFormat::Yaml))
            ...
            .unwrap();
        let res = config.try_deserialize::<Settings>().unwrap();
        assert_eq!(res.controller.listen, "127.0.0.2");
        unsafe { std::env::remove_var("EK_CONTROLLER_LISTEN") };
    }
...
"#;
        ...
        assert_eq!(settings.controller.listen, "0.0.0.0");
    }
}
