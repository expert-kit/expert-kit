...
from expertkit_transport.adapters.grpc import GrpcBatchSpec, GrpcProtocolError
from expertkit_transport.adapters.shm.codec import ExecuteSlot, OpenSession
from expertkit_transport.adapters.shm.memory import SharedMemoryLayout, SharedMemoryRegion
...
def spec() -> GrpcBatchSpec:
    return GrpcBatchSpec(7, 4, 8, 4, 3, 2, torch.float32)
...
def test_session_claims_valid_rows_and_releases_each_generation() -> None:
    ...
    try:
        ...
        session.release(0, 1)
        with pytest.raises(GrpcProtocolError, match="stale"):
...
def test_session_rejects_invalid_network_routing_and_releases_slot() -> None:
    ...
    try:
        with pytest.raises(GrpcProtocolError, match="zero routing weight"):
