...
def test_current_free_memory_must_cover_every_planned_future_allocation() -> None:
    ...
    validate_available_device_memory(
        plan,
        available_bytes_after_fixed_positions=plan.bytes_to_allocate_after_fixed_positions,
    )
    with pytest.raises(ValueError, match="available device memory"):
        validate_available_device_memory(
            plan,
            available_bytes_after_fixed_positions=(
                plan.bytes_to_allocate_after_fixed_positions - 1
            ),
        )
