    ...
    async fn create_then_select_channel(&mut self, eid: ExpertIdRef<'_>) -> EKResult<ChannelMeta> {
        ...
        for node in nodes {
            ...
            match channel.as_str() {
                ...
                "shm" => {
                    if !self.all_shm_channels.contains_key(&node.hostname) {
                        let req_name = format!("ek-shmq-req-{}", node.hostname);
                        let resp_name = format!("ek-shmq-resp-{}", node.hostname);
                        let req_queue = ShmQueue::open(&req_name).ok_or_else(|| {
                            EKError::NotFound(format!("SHM request queue not found: {req_name}"))
                        })?;
                        let resp_queue = ShmQueue::open(&resp_name).ok_or_else(|| {
                            EKError::NotFound(format!("SHM response queue not found: {resp_name}"))
                        })?;
                        self.all_shm_channels.insert(
                            node.hostname.clone(),
                            (
                                Arc::new(Mutex::new(req_queue)),
                                Arc::new(Mutex::new(resp_queue)),
                            ),
                        );
                    }
                    let shm_channel =
                        self.all_shm_channels.get(&node.hostname).ok_or_else(|| {
                            EKError::NotFound(format!(
                                "SHM channels not found for worker {}",
                                node.hostname
                            ))
                        })?;
...
impl ShmqWorkerReq {
    pub fn new(expert_id: ExpertIdRef<'_>, input_tensor: &[u8], traceparent: &str) -> Self {
        static ID: AtomicUsize = AtomicUsize::new(1);

        assert!(expert_id.len() < 64);
        assert!(traceparent.len() < TRACEPARENT_SIZE);
