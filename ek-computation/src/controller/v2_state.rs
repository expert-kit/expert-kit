...
use crate::proto::ek::{
    control::v2::{
        ExpertRoute, ExpertState, ExpertStateKind, RegisterWorkerRequest, RegisterWorkerResponse,
        RouteChange, TargetExpert, TargetExpertListPart, TopologyMessage, TopologySnapshotPart,
        TopologyUpdatePart, WorkerRoute, WorkerRunState, topology_message,
    },
...
impl ControllerV2State {
    ...
    pub async fn heartbeat(
        ...
        let (instance_id, became_live, affected) = {
            let worker = inner
                .workers
                .get_mut(worker_id)
                .expect("validated worker must exist");
        ...
        if became_live && publish_routes(&mut inner, instance_id, affected) {
            notify(&mut inner, &self.changed);
        }
    ...
    pub async fn set_targets(
        ...
        let mut inner = self.inner.write().await;
        ensure_current_start(&inner, worker_id, start_id)?;
        let (instance_id, affected, generation, changed, values) = {
            let worker = inner
                .workers
                .get_mut(worker_id)
                .expect("validated worker must exist");
        ...
        if changed && publish_routes(&mut inner, instance_id, affected) {
            notify(&mut inner, &self.changed);
        }
        Ok(Placement {
            generation,
